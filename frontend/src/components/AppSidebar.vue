  ...
  >
    ...
    <div class="h-16 flex items-center px-4 border-b border-slate-200 dark:border-slate-700 overflow-hidden">
      <div class="flex items-center space-x-3">
        <div class="w-9 h-9 rounded-xl bg-slate-900 dark:bg-sky-500 text-white flex items-center justify-center font-bold text-lg shrink-0 shadow-sm">
          Al
        </div>
    ...
    <div class="flex-1 overflow-y-auto px-3 py-4 space-y-6">
      ...
      <div class="space-y-1">
        ...
        <RouterLink
          :to="{ name: 'worker-dashboard' }"
          class="flex items-center space-x-3 px-3 py-2.5 rounded-xl font-medium transition-colors"
          :class="isActive('worker-dashboard') 
            ? 'bg-slate-900 dark:bg-slate-700 text-white' 
            : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/50'"
